...
import { sentraAcceptInvite, SentraApiError } from '@/lib/sentra/api';
...
export default function AcceptInvite({ lang, dict }: { lang: string; dict: Dict }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await sentraAcceptInvite({ token, name, password });
      setDone(true);
    } catch (err) {
        ...
        >
          {!token ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">{dict.successBody}</p>
              <Link
            ...
            <>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white">{dict.title}</h1>
              </div>
